"""
Daily scraper job for ACT NOI opportunities.

This job runs the ACT NOI scraper in incremental mode and returns
...
async def run_daily_scraper_job(mode: str = "incremental") -> Dict[str, Any]:
    """
    Run the daily scraper job to check for new and updated ACT NOI opportunities.
    ...
    logger.info("=" * 80)
    logger.info("Starting daily scraper job for ACT-NOI", event_type="job_start", mode=mode)
    logger.info("=" * 80)
    
    try:
        scraper = NATOOpportunitiesScraper(config_name="ACT-NOI", use_llm=True)
        ...
        logger.info("=" * 80)
        logger.info(
            f"Daily scraper job completed successfully (ACT-NOI)",
        ...
        logger.error("=" * 80)
        logger.error(
            f"Daily scraper job failed (ACT-NOI): {error_msg}",
...
if __name__ == "__main__":
    ...
    if results['success']:
        print(f"\n✅ Job completed successfully (ACT-NOI)")
        print(f"   New: {len(results['new'])}")
        print(f"   Amended: {len(results['amendments'])}")
        print(f"   Duration: {results['duration_seconds']:.2f}s")
        sys.exit(0)
    else:
        print(f"\n❌ Job failed (ACT-NOI): {results.get('error', 'Unknown error')}")
        sys.exit(1)
