"""
Daily scraper job for ACT RFIP opportunities.

This job runs the ACT RFIP scraper in incremental mode and returns
...
async def run_daily_scraper_job_act_rfip(mode: str = "incremental") -> Dict[str, Any]:
    """
    Run the daily scraper job to check for new and updated ACT RFIP opportunities.
    ...
    logger.info("=" * 80)
    logger.info("Starting daily scraper job for ACT-RFIP", event_type="job_start", mode=mode)
    logger.info("=" * 80)
    
    try:
        scraper = NATOOpportunitiesScraper(config_name="ACT-RFIP", use_llm=True)
        ...
        logger.info("=" * 80)
        logger.info(
            f"Daily scraper job completed successfully (ACT-RFIP)",
        ...
        logger.error("=" * 80)
        logger.error(
            f"Daily scraper job failed (ACT-RFIP): {error_msg}",
...
def run_job_sync_act_rfip(mode: str = "incremental") -> Dict[str, Any]:
    """
    ...
    """
    return asyncio.run(run_daily_scraper_job_act_rfip(mode=mode))
...
if __name__ == "__main__":
    ...
    results = run_job_sync_act_rfip(mode=mode)
    
    if results['success']:
        print(f"\n✅ Job completed successfully (ACT-RFIP)")
        print(f"   New: {len(results['new'])}")
        print(f"   Amended: {len(results['amendments'])}")
        print(f"   Duration: {results['duration_seconds']:.2f}s")
        sys.exit(0)
    else:
        print(f"\n❌ Job failed (ACT-RFIP): {results.get('error', 'Unknown error')}")
        sys.exit(1)
