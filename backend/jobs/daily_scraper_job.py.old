"""
Daily scraper job for automated opportunity updates.
...
async def run_daily_scraper_job(mode: str = "incremental") -> Dict[str, Any]:
    """
    Run the daily scraper job to check for new and updated opportunities.
    ...
    logger.info("=" * 80)
    logger.info("Starting daily scraper job", event_type="job_start", mode=mode)
    logger.info("=" * 80)
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info(
            f"Daily scraper job completed successfully",
        ...
        logger.error("=" * 80)
        logger.error(
            f"Daily scraper job failed: {error_msg}",
